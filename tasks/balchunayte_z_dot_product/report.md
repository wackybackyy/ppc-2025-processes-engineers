...
- Процесс с рангом 0 передаёт всем процессам размер векторов.
- Векторы делятся поровну между процессами (`Scatterv`).
- Каждый процесс получает свой локальный блок данных.
...
### Сбор результата
- Итоговое значение получается вызовом `MPI_Reduce` с операцией `MPI_SUM`.
- Ранг 0 получает окончательный результат.

### Коммуникационный шаблон
- Одно широковещание размера данных.  
- Два Scatterv для фрагментов векторов.  
- Один Reduce для сбора суммы.  
